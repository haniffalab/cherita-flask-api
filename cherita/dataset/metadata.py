...
def match_var_names(
    ...
    matched_df["index"] = matched_df["index"].fillna(matched_df["gene_id"])
    matched_df[COL_NAME] = matched_df[COL_NAME].fillna(matched_df["gene_name"])
    matched_df[INDEX_NAME] = matched_df[INDEX_NAME].fillna(-1)
    return matched_df
